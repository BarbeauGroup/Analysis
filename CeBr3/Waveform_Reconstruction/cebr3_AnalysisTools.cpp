...
void plotWaveformAndPulse(vector<Double_t> waveVec,Double_t nsPerSample,Double_t pulseSample) {
  ...
  TLine pulseLine(pulseSample*nsPerSample,0,pulseSample*nsPerSample,16384.);
...
//-------------DAQ SETTINGS----------------//
  ...
  Int_t scatterer_halfWidth=50;
  Int_t scatterer_rejectThresholdSigma=2.;
...
///////////////////////////PROCESS BPM SIGNALS FIRST////////////////////////////
  ...
  BPMTree->SetMaxTreeSize(200000000000LL);
...
////////////////////////////PROCESS LS SIGNALS NEXT/////////////////////////////
  ...
  analysisTree->SetMaxTreeSize(200000000000LL);
		  ...
		  if (entry%10000==0) {
		    ...
		    waveformBranch->GetEntry(entry);
				...
				if (waveformBaseline != 0) {
					...
					//Get vector of pulses found in waveform. For now, we copy the DAQ trigger settings
					vector<Double_t> pulses = getPulsesFromFIR(waveformVector,0,scattererTrigger_riseTime,scattererTrigger_gapTime,scattererTrigger_firThresh,0.5,scatterer_holdOffSamples,scatterer_FIRsamples);
			
					//If we found a pulse, calculate quantities. Otherwise, still store waveform 
					if (pulses.size() > 0) {  
		        
			    	//Check we can traverse scatterer_preOnsetIntegralSamples+l samples back in time and l samples forward in time. Otherwise, still store waveform 
			    	if ((pulses.at(0)-l > scatterer_preOnsetInterpolationSamples)&&(pulses.at(0)+l < waveformVector.size())) {
					  
					    
				      //plotWaveformAndPulse(waveformVector,nsPerSample,pulses.at(0));
					  
					    //interpolate region where we expect to find onset 
					    vector<Double_t> interpWaveVec;
					    interpWaveVec.clear();
					    interpWaveVec = getTsincVector(waveformVector,0,pulses.at(0)-scatterer_preOnsetInterpolationSamples,scatterer_preOnsetInterpolationSamples,nInterPoints,fTaper,l);
					  
							
					    //Get subsample where waveform crosses 20% max value 
							Double_t interpOnsetSample = getPHVCFDSample(interpWaveVec,0,0,interpWaveVec.size(),scatterer_cfdFraction);
							interpOnsetSample /= nInterPoints;
							interpOnsetSample += pulses.at(0)-scatterer_preOnsetInterpolationSamples;
							scatterer_onsetTime = interpOnsetSample*nsPerSample; //only used for TOF
								 
							//plotWaveformAndPulse(interpWaveVec,0.25,interpOnsetSample);
							
					    //Get onset sample, only use sub-sample timing for TOF
					    Int_t onsetSample = round(scatterer_onsetTime/nsPerSample);
				        ...
				        scatterer_peakHeight = getPHV(waveformVector,0,onsetSample,scatterer_postOnsetIntegralSamples,0);
							  ...
							  scatterer_meanTime = getMeanTime(waveformVector,0,onsetSample-scatterer_preOnsetIntegralSamples,scatterer_preOnsetIntegralSamples+scatterer_postOnsetIntegralSamples,scatterer_peakHeight);
								...
								if (BPM_timingVector.size()>0) {
								  while (bpmIndex < BPM_timingVector.size()) {
									  if (BPM_timingVector[bpmIndex] > scatterer_waveformStartTime+scatterer_onsetTime) {
										  if (bpmIndex > 0) {
